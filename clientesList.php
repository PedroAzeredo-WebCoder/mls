...
$table->addHeader("Documento",           "text-center", "col-3");
...
if ($conn->query($query->getSQL()) && getDbValue($query->getCount()) != 0) {
    foreach ($conn->query($query->getSQL()) as $row) {
        ...
        $table->addCol(mascara(strlen($row["documento"]) == 14 ? "XX.XXX.XXX/XXXX-XX" : "XXX.XXX.XXX-XX", $row["documento"]), "text-center");
