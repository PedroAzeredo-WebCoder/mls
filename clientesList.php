...
if ($conn->query($query->getSQL()) && getDbValue($query->getCount()) != 0) {
    foreach ($conn->query($query->getSQL()) as $row) {
        ...
        if (!empty($row["imagem"])) {
            ...
            $imagem = '<div class="rounded-circle d-flex align-items-center justify-content-center bg-warning mx-auto avatar-sm"><span class="font-28rem text-white">'.$row['nome'][0].'</span></div>';
