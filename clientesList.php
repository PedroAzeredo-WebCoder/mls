...
$table->addHeader("Endereço", "text-center");
...
if ($conn->query($query->getSQL()) && getDbValue($query->getCount()) != 0) {
    foreach ($conn->query($query->getSQL()) as $row) {
        ...
        if (!empty($row["imagem"])) {
            $imagem = '<img src="data:image/png;base64,' . $row['imagem'] . '" class="ratio ratio-1x1 rounded-circle mx-auto avatar-sm">';
        }else{
            $imagem = '<div class="rounded-circle d-flex align-items-center justify-content-center text-black mx-auto avatar-sm"><span class="font-28rem text-white">'.$row['nome'][0].'</span></div>';
        }

        $table->addCol($imagem, "text-center");
        $table->addCol(btn($row['nome'], ["clientesCad.php", ["cad_cliente_id" => $row["id"]]], "btn-link ps-0 fw-normal edit"));
        $table->addCol($row['logradouro'] . ', ' . $row['numero'] . ' - ' . $row['complemento'], "text-center");
        $table->addCol($celular, "text-center");
        $table->addCol($status, "text-center");
        if ($row["status"] != 1) {
            $table->addCol(btn("<i class='fa-regular fa-pen-to-square'></i>", ["clientesCad.php", ["cad_cliente_id" => $row["id"]]], "transparent", "btn-sm btn-outline-danger mx-1 edit") . btn("<i class='fa-solid fa-trash'></i>", ["clientesCadSave.php", ["cad_cliente_id_delete" => $row["id"]]], NULL, "btn-sm edit"), "text-center");
        } else {
            $table->addCol(btn("<i class='fa-regular fa-pen-to-square'></i>", ["clientesCad.php", ["cad_cliente_id" => $row["id"]]], NULL, "btn-sm edit"), "text-center");
        }
        $table->endRow();
    }
} else {
